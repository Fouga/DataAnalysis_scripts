...
input_dataset = datasets.ImageFolder(root='/home/natasha/Programming/Python_wd/single_multiple_bacteria/DataToClassify/', transform=data_transform_1)
input_dataset2 = ImageFolderWithPaths(root='/home/natasha/Programming/Python_wd/single_multiple_bacteria/DataToClassify/', transform=data_transform_1)
...
input_loader = torch.utils.data.DataLoader(input_dataset,
                                             ...
                                             shuffle=False, # set this to true while training!
                                             num_workers=0)
input_loader2 = torch.utils.data.DataLoader(input_dataset2,
                                             batch_size=1,
                                             shuffle=False, # set this to true while training!
                                             num_workers=0)
...
with torch.no_grad():
    ...
    print('Test Accuracy of the model on the training images: {} %'.format(100 * correct / total))
#%% Test Accuracy of the model on the validation
with torch.no_grad():
    correct = 0
    total = 0
    failed = 0
...
#%% apply network to new data

columns = ['imagename','predictions']
newDF = pd.DataFrame() 
inds = 1
with torch.no_grad():
    for images, labels, paths in input_loader2:   

        images = images.to(device)
        #labels = labels.float().to(device)
        outputs = net(images)
        Name = paths[0]
        Pos = findOccurrences(Name,'/')
        filename = Name[Pos[-1]+1:]
        _, predicted = torch.max(outputs.data, 1)
        predicted = torch.reshape(((outputs > t).float() * 1), [1,predicted.size(0)])

        # make a table with the results: filename: prediction    
        #inp =  images[0].to('cpu')
        #inp = inp.numpy().transpose((1, 2, 0))

       # title =  '{}_predicted_{}'.format(train_dataset.classes[predicted[0].int()],filename)
        
     #   scipy.misc.imsave('/home/natasha/Programming/Python_wd/single_multiple_bacteria/results/{}/{}'.format(train_dataset.classes[predicted[0].int()],title), inp)           
       # imageio.imwrite('/home/natasha/Programming/Python_wd/single_multiple_bacteria/results/{}/{}'.format(train_dataset.classes[predicted[0].int()],title), inp)           
        temp = pd.DataFrame({'imagename': filename, 'predictions': train_dataset.classes[predicted[0].int()]},index=[inds])
